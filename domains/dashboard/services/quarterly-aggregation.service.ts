import { QuarterlyAggregateRepo } from "../repositories/quarterly-aggregate.repo";
import { discardNullFields, safeAvg } from "../utils/average.util";

export class QuarterlyAggregationService {
    static async getQuarterly(deviceId: string, dateStr: string) {
        const records = await QuarterlyAggregateRepo.findLast90Days(deviceId, dateStr);
        // console.log(records)
