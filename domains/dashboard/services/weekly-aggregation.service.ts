import { WeeklyAggregateRepo } from "../repositories/weekly-aggregate.repo";
import { discardNullFields, safeAvg } from "../utils/average.util";

export class WeeklyAggregationService {
    static async getWeekly(deviceId: string, dateStr: string) {
        const records = await WeeklyAggregateRepo.findByWeek(deviceId, dateStr);
