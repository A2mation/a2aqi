...
const formSchema = z.object({
  ...
  isActive: z.boolean(),
  parameters: z
    .array(z.string().min(1, "Select a sensor"))
    .min(1, "Add at least one sensor"),
});
  ...
  const form = useForm<DeviceModelFormValues>({
    ...
    defaultValues: {
      ...
      parameters: initialData?.parameters
        ? (initialData.parameters as string[])
        : [""],
    },
  });

  const selectedValues = form.watch("parameters") || [];
    ...
    try {
      ...
      if (initialData) {
        res = await http.patch(
          `/api/admin/device-model/${params.deviceModelId}`,
          data,
        );
      } else {
  ...
  const onDelete = async () => {
    try {
      ...
      if (res.status === 200) {
        ...
        toast.error(
          res.data.message ||
            "Please delete all blogs written by this Device Model first",
        );
            ...
            <div className="space-y-3">
              ...
              {fields.map((field, index) => (
                <div key={field.id} className="flex gap-2 items-start">
                  <FormField
                    control={form.control}
                    name={`parameters.${index}` as const}
                    render={({ field }) => (
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription className="text-red-600">
                    Please elaborately mention which model contains which
                    fields. (EX: AQI, PM2.5, So2 etc.)
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
