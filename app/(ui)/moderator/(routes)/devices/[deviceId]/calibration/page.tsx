  ...
  sensors: z
    .array(
      z.object({
        ...
        value: z.coerce.number("Must be a number"),
      }),
    )
...
export default function DynamicCalibrationPage() {
    ...
    useInfiniteQuery({
      ...
      queryFn: async ({ pageParam = null }) => {
        const { data } = await http.get(
          `/api/moderators/devices/${params.deviceId}/calibration`,
  ...
  const mutation = useMutation({
    mutationFn: async (values: z.infer<typeof calibrationSchema>) => {
      const newValues = values.sensors.reduce(
        (acc, curr) => ({
          ...acc,
          [curr.key]: curr.value,
        }),
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto min-h-screen space-y-6">
      <div className="flex flex-col gap-6">
        <Card className="h-fit">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Settings2 size={20} /> Configure Device
            </CardTitle>
          </CardHeader>
          <CardContent>
            <Form {...form}>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <Input
                        {...field}
                        placeholder="e.g. Sensor Drift Correction"
                      />
                      <FormMessage />
                    </FormItem>
                  )}
        ...
        <Card className="flex flex-col h-150 w-full">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
            ...
            <ScrollArea className="h-full px-4 md:px-6">
              ...
              <div className="md:hidden flex flex-col gap-4 py-4">
                {allLogs.map((log: CalibrationLog) => (
                  <div
                    key={log.id}
                    className="border rounded-xl p-4 space-y-3 bg-slate-50/50 shadow-sm"
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[11px]">
                      <div>
                        ...
                        <p>
                          {new Date(log.effectiveFrom).toLocaleTimeString([], {
                            timeStyle: "short",
                          })}
                        </p>
                      </div>
                      <div>
                        <p className="font-semibold text-red-600">Expires</p>
                        <p>
              ...
              {/* INFINITE SCROLL / LOADING / EMPTY STATES */}
              <div className="py-6">
