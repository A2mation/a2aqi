    ...
    <div className="lg:col-span-2 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
      ...
      <div className="mb-1 pb-4 border-b border-gray-50">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-sm text-gray-400 font-mono font-extrabold uppercase">
            Sync:{" "}
            {liveData.measuredAt
              ? formatDateTime(liveData.measuredAt.toString())
              : "Just now"}
          </span>
        </div>
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-xs text-gray-400 font-medium">
              Toggle metrics to update chart trends
            </p>
          </div>
            ...
            <defs>
              {activeParams.map((param) => (
                ...
                >
                  <stop
                    offset="5%"
                    stopColor={PARAM_CONFIG[param]?.color}
                    stopOpacity={0.15}
                  />
                  <stop
                    offset="95%"
        ...
        {payload.map((item: any) => (
          ...
          >
            ...
            <span className="text-sm font-black text-gray-900">
              {item.value ? Math.round(item.value * 10) / 10 : 0}
            </span>
          </div>
        ))}
      </div>
