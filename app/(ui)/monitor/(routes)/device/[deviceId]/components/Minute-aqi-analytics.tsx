...
export function MinuteAqiAnalytics({
    ...
    return (
        <Card className="p-6 mt-10 backdrop-blur-xl rounded-[3rem]  transition-all duration-500 hover:shadow-xl bg-linear-to-br from-background to-muted/20">
            ...
            <CardFooter className="border-t border-muted/50 flex flex-wrap gap-x-8 gap-y-3">
                {selectedMetricKeys.map((key) => {
                    const config = PARAM_MASTER_CONFIG.find(c => c.id.toLowerCase() === key);
