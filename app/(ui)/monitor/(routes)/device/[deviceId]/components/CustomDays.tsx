...
const CustomDays = ({ 
    ...
    // for 30 days
    const { data: chartData = [], isLoading, isFetching, isError } = useQuery<DaysReading[]>({
        queryKey: ['device-analytics-daywise', deviceId, selectedDays],
        queryFn: async () => {
            const response = await http(`/api/monitor/device/${deviceId}/history?days=${selectedDays}`);
            if (!response.data.success) {
               throw new Error(response.data.message);
            }
            return response.data.history;
        },
        initialData: selectedDays === 30 ? last30days : undefined,
    ...
    return (
        <div className="bg-white/80 mt-10 backdrop-blur-xl rounded-[3rem] p-10 border border-gray-100 shadow-[0_20px_50px_rgba(0,0,0,0.02)] w-full min-h-150">
            ...
            <div className="h-100 w-full group/chart relative">
                <AnimatePresence>
                    {(isLoading || isFetching) && selectedDays !== 30 && (
                        <motion.div 
                            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                            className="absolute inset-0 z-10 flex items-center justify-center bg-white/40 backdrop-blur-sm rounded-3xl"
                        >
